...
/**
 * LinkedIn C2C/Contract Job Scraper (Public Search Approach)
 * 
 * Uses Playwright to fetch and parse the initial HTML of LinkedIn public job search.
 * Filters for contract/C2C positions specifically.
 * 
 * Usage:
...
/**
 * Build LinkedIn public search URL with pagination for C2C/Contract jobs
 */
function buildPublicSearchUrl(keywords, location, start = 0) {
  const encodedKeywords = encodeURIComponent(keywords);
  const encodedLocation = encodeURIComponent(location);
  
  // LinkedIn public job search URL with C2C/Contract filters
  ...
  // contractType: corp (Corp-to-Corp), int (Intern), temp (Temp)
  return `https://www.linkedin.com/jobs/search/?keywords=${encodedKeywords}&location=${encodedLocation}&f_TPR=r604800&jobType=F&contractType=corp&start=${start}`;
}
...
async function scrape() {
  ...
  console.error(`Searching LinkedIn (C2C/Contract) for: "${config.keywords}" in ${config.location}`);
  console.error(`Filters: Freelance/Contract, Last 7 days`);
  ...
  try {
    while (hasMorePages && results.length < config.limit) {
      ...
      for (const job of jobs) {
        ...
        // Build job object with C2C-specific fields
        const jobData = {
          source: 'linkedin_c2c',
          job_id: `linkedin_c2c_${uuidv4().substring(0, 8)}`,
          ...
          job_type: 'C2C/Contract',
          rate: null, // C2C specific - would need to extract from description
